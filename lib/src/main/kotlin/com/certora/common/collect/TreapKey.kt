 ...
 *
 * The treap key is related to, but not the same as, the key used for map entries or set elements.  For map/set
 * keys that implement Comparable, the map/set key is used as the treap key directly.  The ordering defined by the
 * key type determines the treap sort order; priority is determined by a hash of the key's hashCode.  For map/set keys
 * that don't implement a total ordering via Comparable, we bucket keys by hash code.  Buckets sort from left to right
...
internal interface TreapKey<@Treapable K> {
    ...
    open val treapPriority: Int get() {
        // The goal is to produce a seemingly random number using the object's hash code as a seed.  We borrow
        // the proven hash finalization mixing function from MurmurHash:

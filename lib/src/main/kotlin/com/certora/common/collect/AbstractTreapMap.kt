...
internal abstract class AbstractTreapMap<@Treapable K, V, S : AbstractTreapMap<K, V, S>>(
    ...
    /**
     * In order to reduce heap space usage, we derive from Treap.  That makes it tricky to have a AbstractTreapMap representing
    ...
    /**
     * Converts the given Map to a AbstractTreapMap, if the Collection is already a AbstractTreapMap of the same type as 'this'
     * AbstractTreapMap.  For example, if this is a HashTreapMap, and so is the supplied collection.  Otherwise returns null.
     */
    ...
    /**
     * Merges the entries in `m` with the entries in this AbstractTreapMap, applying the "merger" function to get the new values
     * for each key.
     */

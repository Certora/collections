...
internal class ArrayHashTable_Capacity16Bits(
    ...
    private fun indexArray(count: Int) = ShortArray(count)

    private inline fun indexArray(count: Int, init: (Int) -> Int) =
            ShortArray(count) { init(it).toIndex() }

    private fun Int.toIndex() = toShort()
}
